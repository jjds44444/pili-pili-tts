...
# The real centre of the play area - dead centre on a true circle. Nothing
# is placed here and there are deliberately no snap points near it: this is
# where cards get thrown, and snapping fought that (see the snap-point note
# further down).
PLAY_CENTRE = (0.0, 0.0)
...
# about). The discard sits beside the play deck; undealt leftovers just stay
# on the deck rather than getting their own spot (see dealCards()).
...
1. DEAL - press the chilli
   ...
   how many cards to deal (printed bottom-left). Leftovers stay face down
   on the deck - still in play for the one Mission that draws an extra card.
...
def build(missions, urls, out_dir):
    ...
    header = ["-- generated by build_save.py; do not edit, change the Python",
              "SEAT_ORDER = {%s}" % order]
    for name, p in (("POS_PLAY", POS_PLAY), ("POS_MISSION", POS_MISSION),
                    ("POS_REVEAL", POS_REVEAL), ("POS_DISCARD", POS_DISCARD)):
